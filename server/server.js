...
app.post('/convert/epub-to-pdf', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    calibreProcess.stdout.on('data', (data) => {
      ...
      console.log('[EPUB2PDF] Calibre output:', data.toString());
    });

    calibreProcess.stderr.on('data', (data) => {
      stderr += data.toString();
      console.error('[EPUB2PDF] Calibre error:', data.toString());
    });

    calibreProcess.on('close', async (code) => {
      try {
        await fs.unlink(renamedInputPath); // Clean up input file
        
        if (code !== 0) {
          console.error('[EPUB2PDF] Calibre conversion failed with code:', code);
          console.error('[EPUB2PDF] stderr:', stderr);
    ...
    calibreProcess.on('error', (error) => {
      console.error('[EPUB2PDF] Failed to start Calibre process:', error);
      if (!res.headersSent) res.status(500).json({ error: 'Failed to start conversion process', details: error.message });
    });

    calibreProcess.on('timeout', () => {
      console.error('[EPUB2PDF] Calibre process timed out after 5 minutes');
      calibreProcess.kill('SIGKILL');
      if (!res.headersSent) res.status(500).json({ error: 'Conversion timed out. Please try with a smaller file.' });
    });
  } catch (err) {
