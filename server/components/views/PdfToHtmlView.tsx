...
const PdfToHtmlView: React.FC<PdfToHtmlViewProps> = ({ task }) => {
  const performPdfToHtmlConversion = async (
    files: UploadedFile[],
    ...
    formData.append('embedImages', options.embedImages ? 'true' : 'false');
    ...
    try {
      ...
      let filename = `${file.file.name.split('.')[0]}.html`;
    ...
    return [{
      ...
      throw new Error(`PDF to HTML conversion failed: ${errorMessage}`);
    }
  };
  ...
  return (
    <BaseConversionView 
        task={task} 
        performConversion={performPdfToHtmlConversion}
        customValidation={validatePdfFile}
    />
  );
};

export default PdfToHtmlView;