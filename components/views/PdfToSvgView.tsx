...
const PdfToSvgView: React.FC<PdfToSvgViewProps> = ({ task }) => {
  ...
  const performPdfToSvgConversion = async (
    files: UploadedFile[],
    ...
    try {
      ...
      let filename = `${file.file.name.split('.')[0]}.svg`;
  ...
  return (
    <BaseConversionView 
        task={task} 
        performConversion={performPdfToSvgConversion}
        customValidation={customFileValidation}
    />
  );
};

export default PdfToSvgView; 